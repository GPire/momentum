...
export async function accessIdentity(request, env, fetchKeys = fetch) {
  ...
  if (header.alg !== 'RS256' || header.crit !== undefined || typeof header.kid !== 'string' || claims.iss !== env.ACCESS_ISSUER ||
  ...
  const response = await fetchKeys(`${env.ACCESS_ISSUER}/cdn-cgi/access/certs`, { redirect: 'error', signal: AbortSignal.timeout(5000) });
