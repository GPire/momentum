...
- **BUG CRITICO REALE trovato e risolto, segnalato dall'utente**: "quando inserisco lo stipendio e il budget non viene diviso in modo corretto al giorno". Causa trovata leggendo `cycleAllowance`/`commitmentForecast` (`src/predict/fixed-commitments.js`): appena un utente imposta uno STIPENDIO, la Dashboard passa automaticamente a "Il tuo mese, senza sorprese" (Cassa Unica) come UNICA fonte di "oggi puoi spendere" (`cassaUnicaAttiva` in main.js) — ma quel motore calcolava il pool disponibile SOLO da `stipendio − impegni fissi`, **ignorando completamente il budget mensile che l'utente aveva dichiarato**. Risultato verificato dal vivo: stipendio 1800€ + budget dichiarato 1200€ → prima del fix la card avrebbe proposto "liberi" l'intero stipendio (1800€, ~78€/giorno); un budget più prudente veniva silenziosamente ignorato proprio nel momento (stipendio+budget insieme) in cui l'utente si aspetta che i due numeri collaborino. **Fix**: il pool del ciclo ora è il MINIMO fra quanto lascia lo stipendio e la quota del budget mensile che spetta al ciclo di stipendio (proporzionale alla sua lunghezza, media 30,44 giorni/mese — il ciclo raramente coincide col mese di calendario) — mai il numero più permissivo dei due. Retrocompatibile: senza un budget dichiarato il comportamento resta quello di sempre. Verificato dal vivo dopo il fix con gli stessi numeri: "Oggi puoi spendere 53,13 €" su "1222,08 € liberi" (= 1200€ proporzionati al ciclo di 31 giorni), non più 1800€. 4 test nuovi in `fixed-commitments.test.js` (uno per ciascuna direzione: il budget vince quando è più prudente, lo stipendio vince quando il budget dichiarato è più generoso — mai proporre soldi che non arrivano davvero, retrocompatibilità senza budget).
