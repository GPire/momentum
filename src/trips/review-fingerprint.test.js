...
test('attachments, amounts, offered expenses and policy change the fingerprint', async () => {
  const hash = await fingerprint();
  for (const patch of [{ amount: 13 }, { receiptImage: 'replacement' }, { date: '2026-09-14' }, { currency: 'USD' }, { paymentMethod: 'carta' }, { transportMode: 'taxi_ncc' }, { co2: { modo: 'treno', distanzaKm: 30 } }, { originalCurrency: 'CHF', originalAmount: 12, exchangeRate: 0.92 }]) {
