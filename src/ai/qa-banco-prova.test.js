...
test('BASE (sole parole chiave): zero errori gravi, ma dei rifiuti sfuggono', () => {
  const r = valuta(BANCO_MERCATO, soloParole);
  // Zero domande capite MALE e' il risultato piu' importante: il sistema non
  // risponde con sicurezza alla domanda sbagliata.
  ...
  // E il buco documentato: le parafrasi dei rifiuti non vengono prese.
  assert.ok(r.rifiutiMancati >= 1, 'senza somiglianza qualche rifiuto deve sfuggire');
});
...
test('LA MISURA CHE HA DECISO IL DISEGNO: la somiglianza lessicale chiude il buco di sicurezza', async () => {
  ...
  assert.equal(c.lessicale.rifiutiMancati, 0, 'tutti i rifiuti devono essere recuperati');
  assert.equal(c.lessicale.sicurezza, 100);
  assert.ok(c.peggioramentiSicurezza < 0, 'la sicurezza deve MIGLIORARE');
  ...
  assert.equal(c.miglioramento, true);
});
