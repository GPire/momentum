...
export function analyzeCausalStructure(series, {
  ...
  return {
    ...
    edges: effetti.edges,
    scenari: diagnosi.perDecidere ? effetti.scenari : [],
    scenarioStatus: diagnosi.perDecidere ? 'conditional-estimate' : 'blocked-diagnostics',
    scenarioBlockReasons: diagnosi.perDecidere ? [] : (diagnosi.avvertimenti || []).filter(a => a.gravita === 'alta').map(a => a.tipo),
    diagnosi,
    nonLineari,
    testEseguiti: discovered.testEseguiti,
    correzione: discovered.correzione,
    riassunto: [
      gravi > 0 ? diagnosi.riassunto : effetti.riassunto,
