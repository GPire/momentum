...
// `icon` è una CHIAVE SEMANTICA ('urgente'/'attenzione'/'monitoraggio'/
// 'bilancia'/'accantona'/'ok'/'startup'), MAI un'emoji — stessa regola non
...
export function simulateNewPartitaIva(annualInvoiced = 0, opts = {}) {
  ...
  const { setAside, net, cassaNome, cassaCalcolo } = taxSetAside(fatturato, { regime: regimeKey, cassaPropria: opts.cassaPropria, altraCoperturaPrevidenziale: opts.altraCoperturaPrevidenziale, eta: opts.eta, overrides: { ...atecoCoeff, ...opts.overrides } });
  ...
  if (!suggestion.overCeiling && regimeKey === 'forfettario') {
    ...
    const startupCalc = taxSetAside(fatturato, { regime: 'forfettario_startup', cassaPropria: opts.cassaPropria, altraCoperturaPrevidenziale: opts.altraCoperturaPrevidenziale, eta: opts.eta, overrides: { ...atecoCoeff, ...opts.overrides } });
