...
export function cicliText() {
  ...
  return `Nel periodo ${DA}–${A}, con coperture diverse per azioni, metalli, petrolio, terre rare e case, ho trovato ${e.quanti} episodi con la stessa forma: un mercato sale molto, tocca un massimo, poi ne perde almeno un terzo. In media ci mettono ${f.anniDiSalita} anni a salire ${Math.round(f.salitaMedia * 100)}% e ${f.anniDiCaduta} anni a perdere ${Math.abs(Math.round(f.cadutaMedia * 100))}%. ${e.salitaPiuLungaDellaCaduta ? 'La salita dura piu\' della discesa.' : 'La discesa dura piu\' della salita.'}${insieme ? ` L'anno in cui piu' mercati hanno toccato il massimo insieme e' il ${insieme.anno}: ${insieme.mercati.join(', ')}. Questa coincidenza non dimostra una causa: per spiegare il motivo servono fonti sugli eventi di quell'anno.` : ''} Sono episodi riconosciuti a posteriori, non una previsione.`;
}
