...
try {
  const result = await compiler.build({
    entryPoints: ['functions/v1/[[path]].js', 'functions/company/[[path]].js'],
    bundle: true,
    format: 'esm',
    platform: 'browser',
    target: 'es2022',
  ...
  if (result.errors.length || result.outputFiles.length !== 2) throw new Error('Pages Functions bundle incomplete.');
  console.log(`Portable Pages Functions build: ${result.outputFiles.length}/2 routes bundled with esbuild ${compiler.version} WebAssembly.`);
