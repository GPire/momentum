...
import { attachmentRecoveryRequest,recordCleanup } from '../server/company/attachment-recovery.js';
const recoveryMode=process.argv.includes('--recovery');
const storageMode=recoveryMode||process.argv.includes('--storage');
const inboxMode=process.argv.includes('--inbox');
const ownerMode=storageMode||inboxMode||process.argv.includes('--owner');
const port=recoveryMode?4202:storageMode?4201:inboxMode?4197:ownerMode?4194:4193;
const sql = new DatabaseSync(':memory:');
for (const file of ['schema.sql', 'invitations.sql','reports.sql','attachment-quota.sql','attachment-lifecycle.sql','attachment-journal.sql']) sql.exec(readFileSync(new URL(`../server/company/${file}`, import.meta.url), 'utf8'));
...
if(recoveryMode){const key='demo/'+'a'.repeat(64)+'/'+'0'.repeat(64);sql.prepare("INSERT INTO company_attachment_lifecycle(company_id,object_key,state,cleanup_id) VALUES(?,?,'deleting',?)").run('demo',key,'test-attempt');for(const event of ['requested','confirmed','uncertain'])await recordCleanup(db,'demo',key,'test-attempt','owner',event)}
if(inboxMode){
